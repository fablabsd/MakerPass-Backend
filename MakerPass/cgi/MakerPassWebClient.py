...
def getMachineUsageData():
	...
	try:
		...
		sql = """
create temporary table a as select distinct machine_rec.machine_id, machine_description, parent_machine_id, current_state, current_user, last_scan, cast(((julianday(datetime('now', 'localtime')) - julianday(last_scan)) * 1440) as integer) running_time, last_message 
from machine_rec
...
select distinct machine_rec.machine_id, machine_description, parent_machine_id, current_state, current_user, null last_scan, null running_time, last_message
from machine_rec
join smartplug_rec on (smartplug_rec.plug_id = machine_rec.plug_id)
left join user_machine_allocation_rec on (user_machine_allocation_rec.machine_id = machine_rec.machine_id)
...
def printMachineRecTable():
	...
	response += "<tr><th>Machine</th><th>Current User</th><th>Last Scan</th><th>Machine State</th><th>Running Time (min)</th><th>Last Message</th></tr>" 
        for row in rows:
		...
		response += "<tr>"
                response += "<td>" + row['machine_description'] + "</td><td>" + str(row['current_user']) +  "</td><td>" + str(row['last_scan']) + "</td><td>" + row['current_state'] + "</td><td>" + str(running_time) + "</td><td>" + str(row['last_message']) + "</td>"
		response += "</tr>"
	response += "</table>"
